...
export const employeeAdapter: EntityAdapter<EmployeeEntity> = createEntityAdapter<
	EmployeeEntity
>();
...
const employeeReducer = createReducer(
	...
	on(EmployeeActions.loadEmployeeSuccess, (state, { employee }) =>
		employeeAdapter.addAll(employee, { ...state, loaded: true }),
	),
	on(EmployeeActions.loadEmployeeFailure, (state, { error }) => ({
		...state,
		error,
	})),
	on(EmployeeActions.deleteEmployeeFailure, state => initialState),
);
